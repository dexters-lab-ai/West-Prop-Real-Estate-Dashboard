...
with st.sidebar:
    ...
    if st.session_state.is_admin:
        ...
        with st.expander("Admin Login", expanded=False):
            ...
            if st.button("Login"):
                if password == os.getenv("ADMIN_PASSWORD"):  # Use environment variable for admin password
...
def get_profile_recommendations(profile_type, properties_data):
    ...
    profile_mapping = {
        "Diaspora Investor": {
            "filters": lambda x: x.get('name') in ['Radisson Aparthotel', 'Pomona City Flats', 'Millennium Heights'],
            "sort_key": lambda x: -float(x.get('roi', '0%').replace('%', '').split('-')[0]) if x.get('roi', '0%') != 'N/A' else 0
        },
        "Local First-Time Buyer": {
            "filters": lambda x: x.get('name') in ['Pomona City Flats', 'Pokugara Townhouses', 'Millennium Heights'] and \
                               (x.get('name') != 'Millennium Heights' or (float(x.get('price', '$0').replace('$', '').replace('From ', '').replace(',', '').split(' ')[0]) >= 50000 and \
                               ...
                               (any(size in x.get('type', '') for size in ['Studio', '1 Bed', '2 Bed', '3 Bed']) or \
                                (x.get('name') == 'Millennium Heights' and x.get('specs', {}).get('Bedrooms', '') in ['1-3', '1 Bed', '2 Bed'])),
            "sort_key": lambda x: float(x.get('price', '$0').replace('$', '').replace('From ', '').replace(',', '').split(' ')[0])
        },
        "Family Upsizing": {
            "filters": lambda x: x.get('name') in ['Pokugara Townhouses', 'The Hills Lifestyle Estate'],
            "sort_key": lambda x: -float(x.get('price', '$0').replace('$', '').replace('From ', '').replace(',', '').split(' ')[0])
        },
        "Retiree": {
            "filters": lambda x: x.get('name') in ['The Hills Lifestyle Estate'] or any(keyword in x.get('description', '').lower() for keyword in ['peaceful', 'secure', 'low maintenance', 'retirement', 'lifestyle']),
            "sort_key": lambda x: float(x.get('price', '$0').replace('$', '').replace('From ', '').replace(',', '').split(' ')[0])
        },
        "Passive Income Seeker": {
            "filters": lambda x: x.get('name') in ['Radisson Aparthotel', 'Millennium Heights'] or any(keyword in x.get('description', '').lower() for keyword in ['rental', 'income', 'yield', 'investment', 'aparthotel', 'serviced']),
            "sort_key": lambda x: -float(x.get('roi', '0%').replace('%', '').split('-')[0]) if x.get('roi', '0%') != 'N/A' else 0
        }
    }
    ...
    matched = [p for p in properties_data if p and filters['filters'](p)]
...
def create_property_card(property_data, show_contact=False):
    ...
    with st.container():
        # This is a simplified card. The full implementation is complex.
        st.subheader(property_data.get('name', 'N/A'))
        st.write(f"**Price:** {property_data.get('price', 'N/A')}")
        st.write(f"**Status:** {property_data.get('status', 'N/A')}")
...
# --- PROPERTY BROWSER PAGE (NEW) ---
    ...
    if selected_price:
        filtered_properties = filtered_properties[filtered_properties["price"].between(selected_price[0], selected_price[1])]
    if selected_amenities:
        for amenity in selected_amenities:
            filtered_properties = filtered_properties[filtered_properties['amenities'].apply(lambda x: amenity in x if isinstance(x, list) else False)]
    if selected_statuses:
        filtered_properties = filtered_properties[filtered_properties["status"].isin(selected_statuses)]

    # --- Display Filtered Properties ---
    st.markdown(f"#### Showing {len(filtered_properties)} of {len(properties_df)} properties")
    st.markdown("--- ")
...
if 'selected_property' in st.session_state:
    ...
    # Get matches
    matched_properties = get_profile_recommendations(profile, list(property_details.values()))
